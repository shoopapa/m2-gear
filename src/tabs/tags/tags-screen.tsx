...
const TagRoute = ({
  route,
  navigation,
}: NativeStackScreenProps<RootStackParamList, "Tag">) => {
  ...
  const update = async () => {
    try {
      const res = await (API.graphql({
        query: SessionsForTag,
        variables: { tag: tag.id },
      }) as Promise<{ data: SessionsForTagQuery }>);
    ...
    <View style={{ ...globalStyles.container }}>
      <List.Section style={{ width: "100%" }}>
        {getTagData.sessions?.items.map((s) => {
          ...
          if (Date.now() - time.getTime() < 24 * 60 * 60 * 1000) {
            //less than a day show relative
